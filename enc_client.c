...
* Due Date: 3-8-2021
* Description: This will communicate with the enc_server to get the encrypted message.
...
int main(int argc, char* argv[]) {
    ...
    size_t len = 0;
    // Set up the buffers for key
    char* key_buff = calloc((MAX_BUFF) + 1, sizeof(char));
    int key_len = 0;

    // Set up the buffers for input text
    ...
    memset(collected, '\0', MAX_BUFF);
    char* tot_collect = calloc((MAX_BUFF) + 1, sizeof(char));


    // Child variables for fork process
    int childPid = 0;
    int childStatus = 0;
    ...
    switch (spawn_pid) {
        ...
        case 0:
            ...
            // Get length of the plain text
            plain_txt_len = strlen(plain_txt_buff);

            // Make sure there aren't any invalid characters
            for(int i = 0; i < plain_txt_len; i++){
                // Make sure it is a capital letter and not a space
                if((plain_txt_buff[i] < 'A' || plain_txt_buff[i] > 'Z') && plain_txt_buff[i] != ' ') {
                    // Is not a new line
                    if (plain_txt_buff[i] != '\n') {
            ...
            // Gets rid of the new line character at the end
            plain_txt_buff[plain_txt_len - 1] = '0';

            // Read the key information from file
            ...
            if (key_len < plain_txt_len) {
                fprintf(stderr, "Key is too short for text file.\n");
                exit(1);
            }
            ...
            while (strlen(tot_collect) <= plain_txt_len - 1) {
                charsRead = recv(socketFD, collected, 1, 0);
                strcat(tot_collect, collected);
                if (charsRead < 0) {
                    error("CLIENT: ERROR reading from socket");
                }
            }
            // Output the message to stdout
            printf("%s", tot_collect);
            fflush(stdout);
